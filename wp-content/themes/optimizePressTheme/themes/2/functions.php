...
function theme1_setup() {
    ...
    register_nav_menu( 'header_info_bar', __( 'Blog Top Menu', OP_SN ) );
    register_nav_menu( 'primary', __( 'Blog Primary Menu', OP_SN ) );
    register_nav_menu( 'footer', __( 'Footer Copyright Menu', OP_SN ) );
...
function theme1_enqueue_scripts(){
    wp_enqueue_script('theme1-common', OP_THEME_URL.'common'.OP_SCRIPT_DEBUG.'.js', array(OP_SN.'-noconflict-js'), OP_VERSION);
}
    ...
    <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
        <div id="comment-<?php comment_ID(); ?>">
            ...
            <div class="comment-meta cf">
                ...
                <?php comment_reply_link( array_merge( $args, array( 'reply_text' => sprintf(__( '%1$s Reply', OP_SN), '<img src="'.op_theme_img('reply-icon.png',true).'" alt="'.__('Reply',OP_SN).'" width="13" height="9" />'), 'depth' => $depth, 'max_depth' => $args['max_depth'], 'respond_id' => 'leave-reply' ) ) ); ?>
...
function theme1_widgets_init(){
    register_sidebar( array(
        'name' => __( 'Sidebar', OP_SN ),
        'id' => 'main-sidebar',
        'description' => __( 'An optional widget area for your sidebar', OP_SN ),
    ...
    register_sidebar( array(
        'name' => __( 'Sub Footer Section', OP_SN ),
        'id' => 'sub-footer-sidebar',
        'description' => __( 'An optional widget area for your site footer', OP_SN ),
...
function theme1_localize_script($array){
    $array['search_default'] = __('Search...', OP_SN);
    return $array;
}
...
function theme1_list_tags($options=array()){
    ...
    if(count($terms) > 0){
        ...
        foreach($terms as $term){
            $link_start = '<a href="'.get_term_link($term).'" title="'.sprintf(__('View all posts filed under %s', OP_SN), $term->name).'">';
            $out .= '<li><h4>'.$link_start.$term->name.'</a></h4><p>'.$link_start.sprintf(_n('1 Article','%1$s Articles',$term->count,OP_SN), number_format_i18n( $term->count )).'</a></p></li>';
        }
...
function theme1_posts_list($sql,$options=array()){
    ...
    if(count($posts) > 0){
        foreach($posts as $post){
            ...
            $link_start = '<a href="'.$permalink.'" title="'.sprintf( esc_attr__( 'Permalink to %s', OP_SN ), esc_attr($title) ).'" rel="bookmark">';
            ...
            $html .= '
    <li'.$class.'>'.$img.'<h4>'.$link_start.theme1_truncate_title(get_the_title($post->ID)).'</a></h4><p><a href="'.get_comments_link($post->ID).'">'.sprintf(_n('1 Comment','%1$s Comments',$cn,OP_SN), number_format_i18n( $cn )).'</a></p></li>';
        }
...
function theme1_recent_comments($options=array()){
    ...
    if($comments){
        foreach($comments as $comment){
            ...
            $atag = '<a href="'.get_comment_link($comment->comment_ID).'" title="'.sprintf(__('%1$s on %2$s',OP_SN),wp_filter_nohtml_kses($comment->comment_author),$post->post_title).'">';
            $out .= '
    <li>
...
function theme1_color_scheme_fields($color_fields){
    $color_fields = array(
        'start' => array(
            'name' => __('Feature Area Gradient Start Colour', OP_SN),
            'help' => __('Select or enter a colour for the top of the feature area gradient', OP_SN),
        ),
        'end' => array(
            'name' => __('Feature Area Gradient End Colour', OP_SN),
            'help' => __('Select or enter a colour for the bottom of the feature area gradient', OP_SN),
        ),
        'headline_title' => array(
            'name' => __('Page/Category Headline Text Colour', OP_SN),
        )
        /*'top_nav_font' => array(
            'name' => __('Top Navigation Font', OP_SN),
            'help' => __('Choose the font you would like to use for the top navigation bar', OP_SN),
            'type' => 'font'
        ),
        'top_nav_color' => array(
            'name' => __('Top Navigation Background Color', OP_SN),
            'help' => __('Select or enter a colour for the top navigation bar background colour', OP_SN),
        ),
        'link_color' => array(
            'name' => __('Top Navigation Link Color', OP_SN),
            'help' => __('Select or enter a colour for the text link in the top navigation bar', OP_SN),
        ),
        'top_nav_hover_link' => array(
            'name' => __('Navigation Hover Link Text', OP_SN),
            'help' => __('Choose the hover colour for the main text links on your navigation bar', OP_SN),
        ),
        'top_nav_dd_link' => array(
            'name' => __('Dropdown Link Text', OP_SN),
            'help' => __('Choose the colour for the dropdown text links on your navigation bar', OP_SN),
        ),
        'top_nav_dd_hover_link' => array(
            'name' => __('Dropdown Hover Link Text', OP_SN),
            'help' => __('Choose the hover colour for the dropdown text links on your navigation bar', OP_SN),
        ),
        'top_nav_dd' => array(
            'name' => __('Dropdown Background', OP_SN),
            'help' => __('Choose the colour foryour dropdown menu.', OP_SN),
        ),
        'top_nav_dd_hover' => array(
            'name' => __('Dropdown Background Hover', OP_SN),
            'help' => __('Choose the colour for the background on your dropdown menu.', OP_SN),
        )*/
    );
    return $color_fields;
}
