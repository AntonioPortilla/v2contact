...
function op_pagination(array $args=array()){
    ...
    if($pages != 1){
        if($paged > 2 && $paged > $range+1 && $showitems < $pages){
            $out[] = '<li class="first-link"><a href="'.get_pagenum_link(1).'">' . __('Primero', OP_SN) . '</a></li>';
        }
        if($paged > 1 && $showitems < $pages){
            $out[] = '<li class="previous-link"><a href="'.get_pagenum_link($paged - 1).'">' . __('Anterior', OP_SN) . '</a></li>';
        }
        ...
        if($paged < $pages && $showitems < $pages){
            $out[] = '<li class="next-link"><a href="'.get_pagenum_link($cur + 1).'">' . __('Siguiente', OP_SN) . '</a></li>';
        }
        if($paged < $pages-1 && $paged+$range-1 < $pages && $showitems < $pages){
            $out[] = '<li class="last-link"><a href="'.get_pagenum_link($pages).'">' . __('Último', OP_SN) . '</a></li>';
        }
    }
    if(count($out) > 0){
        $out = '
