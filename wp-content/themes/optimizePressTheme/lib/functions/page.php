...
function op_page_footer(){
	...
	if((($footer = op_page_option('footer_area')) !== false) && op_get_var($footer,'enabled') == 'Y'): ?>
		<div class="full-width footer small-footer-text el-pie">
        	<?php
			...
			<div class="row fila-pie">
				...
				<?php
					...
					echo (!empty($disc) && $disclaimer['enabled']=='Y' ? '<small class="footer-disclaimerTWO">'.stripslashes($disc).'</small>' : '');
					//Echo out the nav if it's set
					echo (!empty($footer['nav']) ? '
						<div class="footer-navigationTWO">
							<ul id="nav-footer" class="inline-nav footer-pie">
								'.wp_nav_menu(
