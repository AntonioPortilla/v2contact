...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<p><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php esc_attr( printf( __( 'Return to %s', 'zenite' ), get_the_title( $post->post_parent ) ) ); ?>" rel="gallery"><?php
					/* translators: %s - title of parent post */
					printf( __( '<span>&larr;</span> %s', 'zenite' ), get_the_title( $post->post_parent ) );
				?></a></p>

					<h2><?php the_title(); ?></h2>

						<?php
							printf(__('By %2$s', 'zenite'),
						...
						<span>|</span>
						<?php
							printf( __('Published %2$s', 'zenite'),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf( __( 'Full size is %s pixels', 'zenite'),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										wp_get_attachment_url(),
										esc_attr( __('Link to full-size image', 'zenite') ),
										$metadata['width'],
										$metadata['height']
									)
								);
...
?>
						<p><a href="<?php echo $next_attachment_url; ?>" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php
							$attachment_size = apply_filters( 'zenite_attachment_size', 900 );
