...
function wp_pagenavi( $args = array() ) {
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			// Previous
			if ( $paged > 1 && !empty( $options['prev_text'] ) )//$options['prev_text']
				$out .= $instance->get_single( $paged - 1, 'previouspostslink', '<<' );
			...
			// Next
			if ( $paged < $total_pages && !empty( $options['next_text'] ) )//$options['next_text']
				$out .= $instance->get_single( $paged + 1, 'nextpostslink', '>>' );
...
class PageNavi_Call {
	...
	function get_single( $page, $class, $raw_text, $format = '%PAGE_NUMBER%' ) {
		...
		return "<a href='?paged=" .( $page ). "' class='$class'>$text</a>";
		//return '<a href="?paged="' .( $page ). '"class="'. $class .'">$text</a>"';
	}
